...
class CustomNetwork(ActorCriticPolicy):
    def __init__(self, observation_space, action_space):
        super(CustomNetwork, self).__init__(
            ...
            lr_schedule=lambda _: 0.025, 
            features_extractor_class=CustomFeatureExtractor,
            features_extractor_kwargs={},
            net_arch=[dict(pi=[128, 64, 32], vf=[128, 64, 32])],  # Define separate architectures for policy and value networks
...
class PyBoyEnv(gym.Env):
    def __init__(self, game_path, emunum):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(698,), dtype=np.uint8
        )
    ...
    def step(self, action, ticks=1):
        ...
        if (
            self.emunum == 0 and self.pyboy.frame_count % 1000 == 0
        ) or np.random.randint(77) == 0:
            # obj = Renderer()
            # obj.load_image(
            #     Image.fromarray(
...
def train_model(env, steps):
    ...
    model = PPO(
        policy=CustomNetwork, env=env, policy_kwargs=policy_kwargs, verbose=1, device=device
    )  # Use 'cpu' if GPU is not available
    model.learn(total_timesteps=steps)
    return model


def make_env(game_path, save_state_path, emunum=0, steps=0, result_queue=None):
    def _init():
...
def run_emulator(game_path, save_state_path, emunum=0, steps=0, result_queue=None):
    print("Building Emulator: OS:EMUNUM:", emunum)
    env = make_env(game_path, save_state_path, emunum, steps, result_queue)()
    env.pyboy.load_state(open(save_state_path, "rb"))
    env.pyboy.set_emulation_speed(0)
    # time.sleep(emunum)
    ...
    for i in range(steps):
        print(f"Step: {i} {emunum} started.")
        action, _ = model.predict(obs)
        # print("OS:ACTION:", action)

        actions[i] = action.tolist()
        obs = env.step(action, ticks=ticks)[0]
        print(f"Step: {i} {emunum} complete.")
    print(
        f"{len(actions)} Actions in thread {emunum}:",
        "".join([str(x) for x in actions]),
    )
    env.close()


def run_emulator_mp(game_path, save_state_path, emunum, steps, result_queue, cores):
    print("Building Emulator: OS:EMUNUM:", emunum)
    env = SubprocVecEnv([make_env(game_path, save_state_path, emunum, steps, result_queue)] for emunum in range(cores))

    make_env(game_path, save_state_path, emunum, steps, result_queue)()
...
if __name__ == "__main__":
    ...
    for i in range(num_cores):
        process = multiprocessing.Process(
            target=run_emulator,
            args=(game_path, save_state_path, i, steps, result_queue),
        )
