 ...
 #           "visited_xy": self.visited_xy,
  ...
  #          "seen_and_capture_events": self.seen_and_capture_events,
    ...
    def calculate_reward(self, action=None):
        ...
        reward_map = self.reward_maps[map_id]

        if reward > 0:
            reward_map[px, py] += reward
    ...
    def render(self, target_index=None, reset=False):
        if target_index is not None and target_index == self.emunum or reset:
            ...
            # for y in range(256):
            #     row = [
            #         self.reward_maps.get(self.last_player_map, np.zeros((256, 256), dtype=np.float32))[x, y]
            #         for x in range(256)
            #     ]
            #     points = any(score != 0 for score in row)
            #     if scored == False or points == True:
            #         for x, score in enumerate(row):
            #             if x == self.last_player_x and y == self.last_player_y:
            #                 print("X", end="")
            #             else:
            #                 print(f"{min(9, int(score * 10))}", end="")
            #         print()
            #     if points:
            #         scored = True
    ...
    def render_screen_image(self, target_index=None, reset=False, frame=None, max_frame=None, action=None, other_info=None):
        if target_index is not None and target_index == self.emunum or reset:
            ...
            image = add_string_overlay(image, f"Tile Score: {tile_score:.2f}", position=(20, 140))
            return image
